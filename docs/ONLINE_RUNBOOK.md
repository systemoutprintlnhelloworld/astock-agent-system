...
不带子命令时会进入 `AStock 交互式工作流控制台`。顶层菜单按日常使用路径拆成四个二级页面，降低长命令心智负担：

- **配置向导**：调整 `DATA_MODE`、provider chain、默认 LLM 模型、候选数和历史窗口；通过隐藏输入配置 JQData、iFinD / 同花顺 QuantAPI 凭证；查看脱敏有效配置。
- **数据源诊断**：查看 provider chain 状态，对配置的数据源链做快速 smoke，自定义测试股票、历史天数、检查项和单源超时；小批量同步本地 SQLite 市场库（“撸数据”），再让在线 Agent 优先读本地库。
- **运行工作流**：启动一次智能体工作流，或进入连续运行模式直到 `Ctrl+C` / 达到最大轮数；查看 Agent 状态和停止说明。
