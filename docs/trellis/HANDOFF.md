...
- 2026-07-03 再追加状态：真实后端事件桥接与新闻/公告研究缓存已落地。`apps/backend/app.py` 新增 `BackendAgentEventBridge`，`/api/auto-investment` 与 `/api/auto-investment/background` 会把核心 `AgentEventEmitter` 事件通过 `loop.call_soon_threadsafe()` 转成 WebSocket `make_event()` 广播；`TradingTaskScheduler` 支持可选 `event_emitter` 并兼容旧测试 double。新增 `src/astock_agent_system/data/news_cache.py`、`python -m astock_agent_system.cli datasource news-collect/news-cache` 和 `/api/datasource/news-cache`，默认写入 Git 忽略的 `data/runtime/news_research_cache.jsonl`，递归脱敏 token/password/api key/authorization。该缓存只用于研究证据回放，不是行情 provider、不触发真实下单。聚焦验证为 `python -m pytest tests/test_backend_api.py tests/test_scheduler.py tests/test_data_agent.py tests/test_cli_interactive.py -q`（50 passed），完整 `.\start.bat -Mode delivery-check` 已通过；后续若让 `SentimentAnalyst` 读取 news-cache 需先确认只读研究证据边界。
