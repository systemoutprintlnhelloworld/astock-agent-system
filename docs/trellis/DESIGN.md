...
| 工具 | 当前状态 | 说明 |
| --- | --- | --- |
| context weaver | 已用于本地代码检索 | 用于快速定位端口、Trellis、handoff、UI 启动相关代码。 |
| smart-search | 已尝试，当前 provider 不稳定 | `doctor` 未返回 ok；OpenAI-compatible 诊断遇到上游 503/CLI 异常。不得伪造外部调研。 |
| find-skills | 已搜索 Trellis/handoff | 找到第三方 skill，但安装量较低，暂不安装。 |
| project skill | 已有 `astock-delivery-workflow` | 本轮新增 handoff skill 固化交接流程。 |

## 5. 下一轮设计优先级

推荐顺序：

1. GUI 可诊断性：后端/WS 探测可视化。
2. 拆分 `trading-dashboard.tsx`：按 tab、card、form、chart 分组件。
